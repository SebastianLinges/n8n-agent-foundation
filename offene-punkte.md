...
3. **Zugang — am 04.09.2026 benannt.** Server `az10754sql2`, Datenbank `RWG_SQL_HUB`, Konto `rwg_automate`, Treiber SQLOLEDB. **Das Kennwort steht bewusst nicht im Repo.** Es gehoert in eine n8n-Credential vom Typ *Microsoft SQL* und sonst nirgendwohin. Zwei Dinge sind vor dem ersten Flow zu klaeren: ob dieses Konto **nur lesen** darf - es ist dasselbe, das auch in Microsoft 365 automatisiert, ein Schreibrecht auf dem SQL Hub waere hier unnoetig breit - und ob `statement_timeout` und eine Zeilenobergrenze gesetzt sind. Ein Agent, der freies SQL gegen einen Produktivserver stellt, braucht beides.
   Solange die Credential nicht angelegt ist, kann kein Flow darauf zugreifen; anlegen laesst sie sich nur in der n8n-Oberflaeche.
...
| KI Daily - Analyze | Mo 07.09. 06:20 | Seit dem 04.09. live (`c12ddfb3`): Der Business Scout bekommt 20 statt 5 Kandidaten, und `use_cases aufbereiten` hinterlaesst immer eine Bilanzzeile. **Diese eine Zeile lesen** - sie nennt Kandidatenzahl, Vorschlaege, Verworfene, Parserfehler und den Grund im Klartext. Erwartet: `kandidaten` um 20, und wieder Use Cases in `use_cases`. Kommt weiterhin null, steht der Grund jetzt dort. Zu finden ueber `get_execution` am Knoten `use_cases aufbereiten`, die Bilanzzeile traegt `ist_bilanz` |
...
| Monitor Graph & Teams | Sa 12.09. ab 13:30 | Seit dem 11.09. live (`6baefa3d`): Der Monitor verlaengert das Graph-Abo des Teams-Agenten, damit der nicht nach 72 Stunden verstummt. Die erste Verlaengerung im Betrieb faellt auf den Lauf um 13:30. Erfolgreiche Laeufe werden nicht gespeichert, also per Handlauf nachsehen: `aboAblauf` an `Evaluate Healthcheck` muss danach etwa beim 15.09. stehen statt beim 14.09. 11:23 UTC. Sicherster Beleg ist ohnehin, dass der Agent am Montag nach 13:23 noch antwortet. Kommt stattdessen ein Telegram-Alarm, steht der Grund direkt unter der Kopfzeile |
...
- **Handwerks-Use-Cases — am 04.09. nachgezaehlt.** Die Saeule `handwerk` hat **2 Eintraege, beide verbraucht, 0 offen**. Der Dienstagsslot faellt deshalb auf `fertigung` (5 offen) und `engineering` (14 offen) zurueck. Im ganzen Pool: 52 Use Cases, 43 offen, davon 24 mit Status `new`. Ursache ist die Quellenlage von `KI Daily - Collect`: GitHub, Tavily und Hacker News, also angelsaechsische Tech-Presse, aus der kein deutscher Handwerksprozess entsteht. Vorschlag unveraendert: fuenf bis acht Use-Cases von Hand setzen, das entsperrt den Dienstagsslot sofort. Zu beachten: Sie muessen einen Anker tragen (benanntes Dokument oder benannter Arbeitsschritt), sonst wirft die Themenwahl sie wieder heraus.
...
- **`siem` ohne Wortgrenze im Ingest-Filter — am 04.09. gemessen, heute ohne Schaden.** `Filter Automated Ticket Creator` prueft die Zusammenfassung mit `/(…|siem|…)/i`, was auch „Siemensring“ treffen wuerde. Nachgezaehlt in `jira_tickets`: **64 Tickets** enthalten `siem` in der Zusammenfassung, **alle 64** als eigenstaendiges Wort SIEM, **keines** enthaelt „Siemens“. Alle 64 sind zu Recht aussortiert und haben keine Chunks. Die Haerte auf `\bsiem\b` wie in der Feldpflege waere **regressionsfrei** — die Wortgrenze faengt dieselben 64 — und nimmt eine Falle heraus, die erst beim naechsten Drucker am Siemensring zuschlaegt. Klein, ohne Termin.
